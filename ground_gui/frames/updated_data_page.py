...
class Data_page(tk.Frame):
    ...
    def create_labels(self):
        ...
        tk.Label(self.labels_buttons_frame, textvariable=helios.packet_id).grid(
            row=0, column=1, padx=10, pady=10)
        
        # ------------- Time value + label ------------- #
        tk.Label(self.labels_buttons_frame, text="Time(ms)", height=3).grid(row=1, column=0)
    ...
    def create_angles_labels(self):
        # ------------- Pitch ------------- #
        tk.Label(self.cansat3D_frame,
                 text="Pitch").grid(row=0, column=1)
        tk.Label(self.cansat3D_frame, textvariable=helios.data["AngleX"]).grid(
            row=0, column=2, padx=10, pady=10)
        
        # ------------- Roll ------------- #
        tk.Label(self.cansat3D_frame,
                 text="Roll").grid(row=1, column=1)
        tk.Label(self.cansat3D_frame, textvariable=helios.data["AngleY"]).grid(
            row=1, column=2, padx=10, pady=10)
        
        # ------------- Yaw ------------- #
        tk.Label(self.cansat3D_frame,
                 text="Yaw").grid(row=2, column=1)
        tk.Label(self.cansat3D_frame, textvariable=helios.data["AngleZ"]).grid(
